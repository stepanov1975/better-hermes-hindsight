...
The supported deployment is a trusted Linux Hermes installation with one configured principal and an external Hindsight 0.8.5 or 0.9.1 service. The configured Hindsight credential may have broader server permissions than the plugin exposes; `single_principal` is plugin policy, not server-side authorization.
