...
def test_oversized_current_query_is_rejected_before_hash_or_config(
    ...
    def fail_hash(_query: str) -> str:
        raise AssertionError("oversized query was hashed")

    def fail_config(_home: Path) -> object:
        raise AssertionError("configuration was loaded")
...
def test_planner_bridges_an_asynchronous_session_rotation(tmp_path: Path) -> None:
    ...
    assert mailbox.consume(source_query="question", session_id="parent") == RecallPlan(
        mode="active",
        action="skip",
        rewritten_query=None,
        turn_id="turn-child",
