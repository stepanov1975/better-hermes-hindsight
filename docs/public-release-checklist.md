...
- [x] The package installs and runs on the current stable Hermes release selected by the rolling
      compatibility policy, with no Hermes core patch or Better-owned host fork.
- [x] Provider identity is only `better_hindsight`; bundled `hindsight` remains selectable and
      untouched.
- [x] Current-query recall is bounded, fail-open, redacted, and enabled by default in the normal
      conversation loop.
- [x] Automatic retention is disabled by default and requires explicit profile opt-in on an
      authorized primary handle.
- [x] Enabled retention accepts non-empty user/final-assistant text from the released `sync_turn()`
      callback without origin heuristics.
- [x] Documentation and tests make no direct-user provenance claim and state that durability begins
      only after the provider admission commit.
- [x] Documentation and tests state there is no pre-return or no-loss guarantee for callbacks Hermes
      never invokes or local admissions that fail.
- [x] The callback path performs only bounded local construction and one atomic SQLite transaction;
...
- [x] All-or-none admission, duplicate handling, collision rejection, path confinement, logical row
      and payload caps, SQLite contention, and saturation are tested with sanitized outcomes.
- [x] `retain.segment_max_bytes` cannot exceed `outbox.max_pending_bytes`; initial retry cannot exceed
      maximum retry; every configured queue and retain-deadline number has its documented finite
      ceiling.
- [x] One profile-wide POSIX lock owner recovers and sends rows, including rows admitted by another
      process observed through bounded polling.
- [x] Only rows matching the current destination fingerprint and payload schema are claimed;
      mismatches stay blocked and visible.
- [x] Every retry preserves the same stable document ID and `update_mode="replace"` payload.
- [x] Multi-segment reconstruction metadata is sent through public Hindsight item metadata; shuffled
      remote segments reconstruct the original source and verify its digest after local completion.
- [x] Completion requires the exact audited synchronous response predicate. Timeout, malformed
      response, process stop, or ambiguous remote completion leaves the row retryable.
- [x] The product claims replace-safe replay, not exactly-once transport or global FIFO.
- [x] Bounded shutdown stops new work and leaves unconfirmed admitted rows recoverable.

## Trust, credentials, and missions

- [x] Recall formatting identifies content as potentially stale, untrusted historical evidence.
- [x] Deterministic high-confidence redaction runs before recall egress and retention admission; its
      limitations are documented without claiming universal secret detection.
- [x] API keys remain environment-only, secret-bearing JSON keys are rejected, and public errors,
      representations, tests, and reports expose no secrets or private destination values.
- [x] Exact principal authorization, `single_principal`, `shared`-scope gating, and primary-context
      retain gating pass focused tests.
- [x] Hindsight OSS 0.8.5 shared-key risk is explicit; missing tools and confirmation flags are not
      represented as server-enforced authorization.
- [x] Retain and observation mission text remain distinct. Check/apply is an explicit operator action,
      initialization does not apply policy, and apply requires confirmation plus readback.

## Isolated development proof

- [x] Follow [development-instance.md](development-instance.md); it defines the environment but does
      not provision Hindsight, Docker, an interpreter, a datastore, or a credential.
- [x] Fake HTTP, guard, sanitized-subprocess, and fault tests pass before any live write.
- [x] Development writes use a dedicated Hermes installation/interpreter and temporary profile plus
...
- [x] The live-proof process contains no production URL, API key, bank ID, or profile state; inherited
      generic `HINDSIGHT_*` values are removed and only explicit development inputs are admitted.
- [x] `BETTER_HINDSIGHT_ALLOW_DEV_WRITES=1`, literal loopback or exact development-endpoint allowlist,
...
- [x] The separate development key and datastore remain single-writer for the complete proof because
...
- [x] One explicitly enabled, dedicated-marker proof exercises released-Hermes provider
      discovery/selection,
...
- [x] The same bounded proof performs one fixed usefulness/provenance recall check without aggregation,
...
- [x] The child never deletes the bank: success and failure leave the durable marker for its parent.
...
- [x] Canary activation is separately authorized after isolated proof and evaluates recall usefulness
      plus retained-source quality without mutating the old bank.
- [x] Rollback is not called configuration-only: it stops every Hermes process sharing the
...
- [x] Returning to Better explicitly reinstalls the wheel and `hindsight-client==0.8.5`, installs the
      reviewed Git plugin, verifies discovery before selection/restart, and does not migrate or delete
      either bank/outbox.
- [x] Released `hermes plugins remove` owns only the Git plugin directory; profile configuration,
...
- [x] Better Hindsight code, artifacts, and the complete runtime dependency closure rooted at
      `hindsight-client==0.8.5` pass their blocking security checks.
- [x] Build or publication tooling findings are resolved when they can compromise the candidate
      artifact or publication process.
- [x] Every actively supported Hermes compatibility environment passes its required lifecycle suite.
- [x] A host finding blocks only when the plugin imports or invokes the affected component, exposes
      the affected path, or materially aggravates its risk.
- [x] Hermes `v2026.8.3`'s `cryptography==48.0.1` findings (`PYSEC-2026-3552`,
...
- [x] The complete supported-host audit remains visible as a non-blocking advisory observation;
      failures to execute or collect that audit still fail its diagnostic job.
- [x] A new stable Hermes release updates the required lifecycle lane, or the documented minimum
      compatible version is raised, before public release.

## Compatibility and artifacts

- [x] The current stable Hermes release, historical characterization lanes, Hindsight server/client,
      Python, and POSIX platform versions are documented and tested.
- [x] Imports, discovery, availability checks, and configuration loading perform no network call,
      package installation, bank mutation, or service restart.
- [x] Host-managed Git plugin installation and fresh-process provider/CLI discovery pass in a
      disposable repository and temporary `HERMES_HOME`.
- [x] Explicit Better `0.8.5` and bundled `0.6.1` SDK states are exercised through the documented
      `uv pip --python` transition after every process sharing the interpreter is stopped.
- [x] Wheel/sdist metadata, packaged provider/CLI resources, license, docs, and third-party notices
      are verified without freezing host source bytes or package-manager internals.
- [x] Pre-alpha warnings are replaced with accurate prerelease status and install instructions.

## Public safety and repository health

- [x] Full history and candidate artifacts pass Gitleaks and repository-configured Semgrep rules.
- [x] Public files contain no private endpoints, credentials, bank IDs, principal IDs, paths,
      memories, transcripts, databases, logs, or generated runtime state.
- [x] README, design, configuration, compatibility, operations, rollback, examples, release notes,
      and generated diagnostics agree on the best-effort boundary.
- [x] `uv lock --check`, full pytest, Ruff lint, Ruff formatting, mypy, build, Twine, archive
      inspection, fresh installs, the rolling compatibility matrix, fake-server faults, security
