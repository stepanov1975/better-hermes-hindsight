...
- Added a default-off context-aware automatic-recall planner that runs through Hermes's public
  ...
  session/turn identity, every hook publishes the newest pending fence before configuration or model work.
  Pending reservations fence out late hook workers before provider consumption; publication deadlines are
  sampled and enforced while holding the registry lock, and finalized decisions are consume-once. Legacy
  branch-preview SQLite settings remain loadable but no longer configure the handoff. The runtime does not
  mutate those paths; the documented upgrade requires stopping profile users before explicit offline file
  cleanup.
